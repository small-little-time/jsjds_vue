<template>
  <div>
    <div><h3 style="text-align:center">此页面主要用来配置比赛报名学院专业信息</h3></div>
    ...
    <add-or-update
      ...
      @refreshCategoryList="getAllAcademy"
    />
  </div>
</template>

<script>
import AddOrUpdate from './academyConfig-add-or-update'
import { getAcademy, getAllAcademy, deleteAcademy } from '@/api/info'
  ...
  methods: {
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        deleteAcademy(ids).then((response) => {
